...
const dailyWisdomVerses = [
  ...
  },
  {
    ...
    verseHi: "जय हनुमान ज्ञान गुण सागर",
    source: "Hanuman Chalisa",
  },
  {
...
export async function GET(request: Request) {
  ...
  if (type === "daily-wisdom") {
    const today = new Date()
    const dayOfYear = Math.floor(
      (today.getTime() - new Date(today.getFullYear(), 0, 0).getTime()) / 86400000
    )
    const verseIndex = dayOfYear % dailyWisdomVerses.length
    return NextResponse.json(dailyWisdomVerses[verseIndex])
  }
...
export async function POST(request: Request) {
  ...
  if (action === "subscribe" && subscription) {
    await prisma.notificationPreference.upsert({
      ...
      update: { dailyWisdom: true, eventAlerts: true, streakRemind: true },
      create: {
